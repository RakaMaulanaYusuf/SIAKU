...
<div class="bg-gray-50 min-h-screen flex flex-col" x-data="{ 
    rows: JSON.parse(localStorage.getItem('aktivatetapRows') || '[]'),
    newRow: {
        code: '',
        name: '',
        amount: '',
    ...
    init() {
        if (!this.rows.length) {
            this.rows = [
                { code: '61', name: 'Beban Listrik', amount: '50', isEditing: false },
                { code: '62', name: 'Beban Air', amount: '50', isEditing: false }
            ];
            this.saveData();
        }
    },
    saveData() {
        localStorage.setItem('aktivatetapRows', JSON.stringify(this.rows));
        localStorage.setItem('totalAktivatetap', this.getTotal());
    },
    getTotal() {
        return this.rows.reduce((sum, row) => sum + (Number(row.amount) || 0), 0);
    },
    getTotalAktiva() {
        const totalAktivalancar = Number(localStorage.getItem('totalAktivalancar')) || 0;
        const totalAktivatetap = this.getTotal();
                    ...
                    <table class="min-w-full">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 text-sm">
                            ...
                            <tr class="bg-gray-50 text-black">
                                <td colspan="2" class="py-2 px-4 border font-medium text-center">JUMLAH AKTIVA TETAP</td>
                                <td class="py-2 px-4 border"></td>
                                <td class="py-2 px-4 border text-right font-medium" x-text="formatNumber(getTotal())"></td>
                                <td class="py-2 px-4 border"></td>
                            </tr>
                            <tr class="bg-gray-50 text-black">
                                <td colspan="2" class="py-2 px-4 border font-medium text-center">JUMMLAH AKTIVA</td>
                                <td class="py-2 px-4 border"></td>
                                <td class="py-2 px-4 border text-right font-medium" x-text="formatNumber(getTotalAktiva())"></td>
                                <td class="py-2 px-4 border"></td>
                            </tr>
                        </tbody>
                    </table>
