...
<div class="bg-gray-50 min-h-screen flex flex-col" x-data="{ 
    rows: JSON.parse(localStorage.getItem('operasionalRows') || '[]'),
    newRow: {
        code: '',
        name: '',
        amount: '',
    ...
    init() {
        if (!this.rows.length) {
            this.rows = [
                { code: '61', name: 'Beban Listrik', amount: '100', isEditing: false },
                { code: '62', name: 'Beban Air', amount: '75', isEditing: false }
            ];
            this.saveData();
        }
    },
    saveData() {
        localStorage.setItem('operasionalRows', JSON.stringify(this.rows));
        localStorage.setItem('totalOperasional', this.getTotal());
    },
    getTotal() {
        return this.rows.reduce((sum, row) => sum + (Number(row.amount) || 0), 0);
    },
    getLabaBersih() {
        const totalPendapatan = Number(localStorage.getItem('totalPendapatan')) || 0;
        const totalHPP = Number(localStorage.getItem('totalHPP')) || 0;
        const totalOperasional = this.getTotal();
        
        return totalPendapatan - (totalHPP + totalOperasional);
    },
    formatNumber(number) {
        ...
        <div id="main-content" class="relative text-black ml-72 font-poppins w-full h-full overflow-y-auto">
            ...
            <div class="bg-white p-6 mx-6 mt-6 rounded-xl shadow-sm">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h1 class="text-2xl font-bold text-black">Biaya Operasional</h1>
                    ...
                    <table class="min-w-full">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 text-sm">
                            ...
                            <tr class="bg-gray-50 text-black">
                                <td colspan="2" class="py-2 px-4 border font-medium text-center">JUMLAH BIAYA OPERASIONAL</td>
                                <td class="py-2 px-4 border"></td>
                                <td class="py-2 px-4 border text-right font-medium" x-text="formatNumber(getTotal())"></td>
                                <td class="py-2 px-4 border"></td>
                            </tr>
                            <tr class="bg-gray-50 text-black">
                                <td colspan="2" class="py-2 px-4 border font-medium text-center">LABA BERSIH</td>
                                <td class="py-2 px-4 border"></td>
                                <td class="py-2 px-4 border text-right font-medium" x-text="formatNumber(getLabaBersih())"></td>
                                <td class="py-2 px-4 border"></td>
                            </tr>
                        </tbody>
                    </table>
